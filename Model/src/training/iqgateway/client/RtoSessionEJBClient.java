...
public class RtoSessionEJBClient {
    public static void main(String [] args) {
        try {
            ...
            RtoSessionEJB rtoSessionEJB = (RtoSessionEJB)context.lookup("TmsApp-Model-RtoSessionEJB#training.iqgateway.services.RtoSessionEJB");
            for (Offense offense : (List<Offense>)rtoSessionEJB.getOffenseFindAll()) {
                printOffense(offense);
            }
            for (VehicleApplication vehicleapplication : (List<VehicleApplication>)rtoSessionEJB.getVehicleApplicationFindAll()) {
                printVehicleApplication(vehicleapplication);
            }
            for (Vehicle vehicle : (List<Vehicle>)rtoSessionEJB.getVehicleFindAll()) {
                printVehicle(vehicle);
            }
            for (OffenseDetails offensedetails : (List<OffenseDetails>)rtoSessionEJB.getOffenseDetailsFindAll()) {
                printOffenseDetails(offensedetails);
            }
            for (Owner owner : (List<Owner>)rtoSessionEJB.getOwnerFindAll()) {
                printOwner(owner);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
